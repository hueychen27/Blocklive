...
async function onTabLoad() {
	...
	// trap vm and store
	await waitForElement('div[class^="stage-wrapper_stage-wrapper_"]')
	const script = document.createElement("script");
	script.innerText = `
const myCustomEvent = new CustomEvent('getVm', {
	detail: { key: Object.values(document.querySelector('div[class^="stage-wrapper_stage-wrapper_"]')) }
});
document.dispatchEvent(myCustomEvent);`
	let thing = waitForEvent("getVm");
	if (!!document.head) {
	...
	thing.then(async (e) => {
		let reactInst = e.key.find(x => x.child);
